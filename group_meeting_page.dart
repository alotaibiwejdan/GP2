...
class _GroupMeetingPageState extends State<GroupMeetingPage> {
  ...
  // البيانات الستاتيك (تظهر فقط عندما يكون الـ appointmentId فارغاً)
            ...
            onPressed: () async {
              if (_emailController.text.isNotEmpty) {
                if (widget.appointmentId.isNotEmpty) {
                  // تحديث قائمة المشاركين في فايربيس للموعد الجديد
  ...
  Widget build(BuildContext context) {
    // حالة الموعد الستاتيك (التصميم الثابت المرتب)
    ...
    // حالة الموعد الجديد (جلب البيانات من فايربيس)
    return StreamBuilder<DocumentSnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        if (!snapshot.hasData || !snapshot.data!.exists) {
          return const Scaffold(body: Center(child: Text("الموعد غير موجود")));
        }

        var data = snapshot.data!.data() as Map<String, dynamic>?;
        if (data == null) return const Scaffold(body: Center(child: Text("خطأ في البيانات")));

        // معالجة خطأ النوع (Iterable/String) - التأكد أن المشاركين قائمة
        List<String> participants = [];
        var rawParticipants = data['participants'];
        if (rawParticipants is List) {
          participants = List<String>.from(rawParticipants);
        } else if (rawParticipants is String) {
          participants = [rawParticipants];
        }

        return _buildMainUI(
          title: data['title']?.toString() ?? "موعد جماعي",
          dateInfo: "${data['date'] ?? ''} - ${data['time'] ?? ''}",
          locationInfo: data['location']?.toString() ?? data['place']?.toString() ?? "لم يحدد الموقع",
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Image.asset('assets/images/logo.png', height: 40, 
            errorBuilder: (context, error, stackTrace) => const Text("مرسال", style: TextStyle(color: Color(0xFFC875A8), fontWeight: FontWeight.bold))),
          centerTitle: true,
          actions: [
            IconButton(icon: const Icon(Icons.arrow_forward, color: Colors.grey), onPressed: () => Navigator.pop(context)),
          ],
        ),
        body: Column(
          children: [
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text("(${participants.length}) المستلمون", 
  ...
  Widget _buildUserCard(String email, String status, Color color, bool arrived) {
    return Container(
      ...
      child: ListTile(
        ...
        trailing: Stack(
          ...
          children: [
            const CircleAvatar(radius: 24, backgroundColor: Color(0xFFF5F6F8), child: Icon(Icons.person, color: Colors.grey)),
            if (arrived) 
              const Positioned(bottom: 0, right: 0, child: CircleAvatar(radius: 8, backgroundColor: Colors.green, child: Icon(Icons.check, size: 10, color: Colors.white))),
          ],
        ),
      ),
    );
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: InkWell(
              onTap: () {
                ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تم إرسال تذكير للمستلمين')));
              },
              child: Container(
                height: 55,
                decoration: BoxDecoration(
                ...
                child: const Center(child: Text("إرسال تذكير للجميع", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold))),
              ),
            ),
          ),
        ],
